...
function check(n: any, cls: string) {
  ...
  } else if (cmss.isCond(n)) {
    expect(cmss.isBase(n.parent) || cmss.isElem(n.parent)).toBe(true);
    if (cmss.isElem(n.parent)) {
      expect(cmss.isBase(n.parent.parent)).toBe(true);
    }
    if (!n.off) {
      if (n.mapped) {
...
describe("NSS", () => {
  describe.each([
    ...
    beforeAll(() => {
      n = cmss<typeof Base, typeof Elem, typeof Cond>(Base, Elem, Cond);
    });
  ...
  describe.each([
    ...
    (_, Base, Elem, Cond, BaseMapped, ElemMapped, CondMapped) => {
      ...
      test("enum values", () => {
        verifyBasicForms(
          cmss<typeof BaseMapped, typeof ElemMapped, typeof CondMapped>(
            BaseMapped,
            ElemMapped,
            CondMapped
          )
      ...
      test("map object", () => {
        verifyBasicForms(
          cmss<typeof Base, typeof Elem, typeof Cond>(Base, Elem, Cond, {
            Ship: "abc",
            engine: "def",
            part: "ghi",
            warp: "jkl",
  ...
  describe.each([
    ...
    test("case-sensitive", () => {
      cmss.configure({ caseSensitiveProps: true });
      check(n.props(["Warp"]), "Ship");
      check(n.props({ Warp: true }), "Ship");
      cmss.configure({ caseSensitiveProps: false });
    });
  });
  ...
  describe.each([
    ...
    (_, Base, Elem, Cond, BaseMapped, ElemMapped, CondMapped) => {
      test("omit base name", () => {
        const n = cmss<object, typeof ElemMapped, typeof CondMapped>(
          {},
          ElemMapped,
          CondMapped
        );
      ...
      test("omit elements", () => {
        const n = cmss<typeof BaseMapped, object, typeof CondMapped>(
          BaseMapped,
          null,
          CondMapped
        );
      ...
      test("omit elements + class mappings via map object", () => {
        const n = cmss<typeof Base, object, typeof Cond>(Base, null, Cond, {
          Ship: "abc",
          warp: "jkl",
          // adrift omitted -- custom classes should be optional
        });
      ...
      test("omit conditionals", () => {
        const n = cmss<typeof BaseMapped, typeof ElemMapped>(
          BaseMapped,
          ElemMapped
        );
      ...
      test("omit conditionals + class mappings via map object", () => {
        const n = cmss<typeof Base, typeof Elem>(Base, Elem, null, {
          Ship: "abc",
          engine: "def",
          part: "ghi",
        });
  ...
  describe.each([
    ...
    test("separators", () => {
      ...
      let n = cmss<typeof Base, typeof Elem, typeof Cond>(Base, Elem, Cond);
