...
export default function cmss<Base = object, Elem = object, Cond = object>(
  ...
  function buildCondObjects(
    ...
    return Object.fromEntries(
      Object.entries(cond ?? {}).map(([condName, condClass]) => {
        ...
        function cmssCondObject(on?: unknown) {
          ...
          Object.assign(cmssObject, buildCondObjects(cls, classPrefix, parent));
          return cmssObject;
        }

        cmssCondObject.off = false;
        cmssCondObject.cls = priorClasses_ + classPrefix + condName + _mapped;
        cmssCondObject.c = cmssCondObject.cls; // alias
        cmssCondObject.value = classPrefix + condName;
        cmssCondObject.mapped = mapped;
        cmssCondObject.toString = toStringError;
        cmssCondObject.parent = parent;

        // Set n.cond.name:
        Object.defineProperty(cmssCondObject, "name", {
          value: condName,
          writable: false,
        });

        return [condName, cmssCondObject];
      })
    );
  }

  const cmssElemObjects = Object.fromEntries(
    Object.entries(elem ?? {}).map(([elemName, elemMappedClass]) => {
      ...
      function cmssElemObject(...args: NSSArg<Base, Elem, Cond>[]) {
        return constructNSSObject<Base, Elem, Cond>({
          name: elemName,
          cls: elemClass,
          separator: conditionalSeparator,
          base: cmssBaseObject,
          builder: cmssElemObject,
          mapped,
          mappings,
          mappingsLowercase,
          values: args,
      ...
      cmssElemObject.cls = elemClass + _mapped;
      cmssElemObject.c = cmssElemObject.cls; // alias
      cmssElemObject.value = elemClass;
      cmssElemObject.mapped = mapped;
      cmssElemObject.toString = toStringError;
      cmssElemObject.parent = cmssBaseObject;
      cmssElemObject.props = function (...args: NSSArg<Base, Elem, Cond>[]) {
        return constructNSSObject<Base, Elem, Cond>({
          name: elemName,
          cls: elemClass,
          separator: conditionalSeparator,
      ...
      Object.assign(
        ...
        buildCondObjects(
          ...
          cmssElemObject as unknown as NSSObject<Base, Elem, Cond>
        )
      );

      // Set n.elem.name:
      Object.defineProperty(cmssElemObject, "name", {
        value: elemName,
        writable: false,
      });

      return [elemName, cmssElemObject];
    })
  );
  ...
  function cmssBaseObject(...args: NSSArg<Base, Elem, Cond>[]) {
    return constructNSSObject<Base, Elem, Cond>({
      name: baseNameStr,
      cls: baseNameStr,
      separator: conditionalSeparator,
      base: cmssBaseObject,
      builder: cmssBaseObject,
      mapped,
      mappings,
      mappingsLowercase,
      values: args,
  ...
  cmssBaseObject.cls = baseNameStr + _mapped;
  cmssBaseObject.c = cmssBaseObject.cls; // alias
  cmssBaseObject.value = baseNameStr;
  cmssBaseObject.mapped = mapped;
  cmssBaseObject.toString = toStringError;
  cmssBaseObject.parent = cmssBaseObject;
  cmssBaseObject.props = function (...args: NSSArg<Base, Elem, Cond>[]) {
    return constructNSSObject<Base, Elem, Cond>({
      name: baseNameStr,
      cls: baseNameStr,
      separator: conditionalSeparator,
  ...
  // Set n.name:
  Object.defineProperty(cmssBaseObject, "name", {
    value: baseName,
    writable: false,
  });
  ...
  // eg. n.Ship.s
  if (baseName) {
    Object.defineProperty(cmssBaseObject, baseName, {
      value: cmssBaseObject,
      writable: false,
    });
  }
  ...
  // eg. n.engine.s
  Object.assign(cmssBaseObject, cmssElemObjects);
  ...
  Object.assign(
    ...
    buildCondObjects(
      ...
      cmssBaseObject as unknown as NSSObject<Base, Elem, Cond>
    )
  );

  return cmssBaseObject as unknown as NSS<Base, Elem, Cond>;
}

// resolveNSSArg maps basic cond expressions (eg. n.myCond) to their corresponding
// cmssd cond expressions (eg. n.myElem.myCond) when composing conditionals:
// n.myElem(n.myCondA, n.myCondB)
// This obviates the need to supply fully-cmssd conditionals in this case, eg.
...
export function resolveNSSArg<Base, Elem, Cond>(
  ...
  if (isNSSObject(arg)) {
    ...
    let cmssObj = cond as NSSObject<Base, Elem, Cond>;
    if (!cond) {
      cmssObj = arg as NSSObject<Base, Elem, Cond>;
    } else if ((arg as { off: boolean }).off) {
      return "";
    }
    const { cls, mapped } = cmssObj;
    return cls.slice(
...
cmss.config = config;
cmss.configure = configure;
cmss.isInstance = isNSSObject;
cmss.isBase = isBaseNSSObject;
cmss.isElem = isElemNSSObject;
cmss.isCond = isCondNSSObject;
